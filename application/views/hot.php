   ...
   <div class="container-fluid">
       ...
       <div class="d-sm-flex align-items-center justify-content-between mb-4">
           <div class="container-fluid">
               <center>
                   <h1 class="h3 mb-2 text-gray-800">DATA HOT</h1>
               </center>
               ...
               <table class="table table-bordered table-striped alert alert-dark">
                   ...
                   <?php $nomor = 1; ?>
                   <?php foreach ($hot as $per) : ?>
                       <tr>
                           <td><?= $nomor++ ?></td>
                           <td> <img src="<?= base_url() . '/uploads/' . $per->gambar ?>" width="100">
                           <td><?= $per->judul ?></td>
                           <td><?= $per->artikel ?></td>
                           <td><?= $per->kategori ?></td>
                           <td><?= $per->nama_penulis ?></td>
                           <td><?= $per->tanggal ?></td>

                           <td>
                               <?= anchor('admin/dashboard_admin/edit/' . $per->id, '<div class="btn btn-primary btn-sm"><i class="fas fa-edit"></i></div>') ?>

                               <?= anchor('admin/dashboard_admin/hapus/' . $per->id, '<div class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></div>') ?>
                           </td>
                       </tr>
                   <?php endforeach; ?>
               </table>
